...
| GET | `/api/v1/welds/{weld_id}/versions/{version_id}/calibration` | 读该焊缝的**权威标定**（2026-09-22）。**归属固定钉在该焊缝的 v1.0 原始版本**——对齐任务可能跑在任意加工版本，标定若跟着任务版本走会散出多份互相矛盾的标定；故 `version_id` 只用于焊缝/版本归属校验，任何属于该焊缝的版本都读到同一份。响应 `{calibration, anchored_version_id, video{offset_seconds,calibrated}, seam_image{roi,calibrated,object_key}}` | 需登录并校验焊缝归属 |
| PUT | `/api/v1/welds/{weld_id}/versions/{version_id}/calibration` | 写标定（2026-09-22），写审计（`update`/`calibration`）。**只改 `data_versions.calibration` 一列**——不碰 `object_keys`、不重算历史 `alignment_tasks.mapping`、不动任何 `split_tasks`；重标定只影响此后新发起的对齐/分段。**合并语义**：只更新给出的组，省略的组保持原值，显式 `null` 清除该组。校验：`offset_seconds` 有限且 ≤±3600s（手误护栏）；`roi` 四数有限、`w/h>0`、**且必须落在焊缝照片真实像素范围内**（写入前下载图片读一次宽高，读不到则 400 拒绝） | body: `{video?: {offset_seconds}, seam_image?: {roi:{x,y,w,h}}} \| null` |
