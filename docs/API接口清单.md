...
| `DataRecord` | 焊缝数据登记（一条焊缝 = 一条记录） | id, weld_id, weld_name, registration_no, source, collected_at, machine, weld_method, material, thickness, current_voltage, sample_rate, **wire_feed_speed / welding_speed**(单值工艺参数，登记可填/CSV 导入稳态回填), **data_fields**(JSON 字段概览 `[{id,name,unit,value}]`，导入自动写), product, **dataset_id**(归属数据集 FK, 登记必填), modalities, quality, operator, storage_bytes, latest_version_id |
...
| `Calibration` | 统一坐标系标定（**归属焊缝 v1.0 原始版本**） | calibration{video{offset_seconds}, seam_image{roi{x,y,w,h}, excluded}}, anchored_version_id, video{offset_seconds, calibrated}, seam_image{roi, excluded, calibrated, object_key}。`offset_seconds` 语义：视频零点在信号轴上的时刻，`t_video = t_signal - offset`。`seam_image.excluded=true` = 用户**明确选择**「不对焊缝图片进行分段」（与"还没框 ROI"是两件事：前者是已完成的决定、不再给标定引导，后者是未完成的标定、必须给引导）；两者都让该模态在分段预览与样本 manifest 中记 `available=false` + 原因 |
...
| GET | `/api/v1/welds/{weld_id}/versions/{version_id}/calibration` | 读该焊缝的**权威标定**（2026-09-22）。**归属固定钉在该焊缝的 v1.0 原始版本**——对齐任务可能跑在任意加工版本，标定若跟着任务版本走会散出多份互相矛盾的标定；故 `version_id` 只用于焊缝/版本归属校验，任何属于该焊缝的版本都读到同一份。响应 `{calibration, anchored_version_id, video{offset_seconds,calibrated}, seam_image{roi,excluded,calibrated,object_key}}` | 需登录并校验焊缝归属 |
| PUT | `/api/v1/welds/{weld_id}/versions/{version_id}/calibration` | 写标定（2026-09-22），写审计（`update`/`calibration`）。**只改 `data_versions.calibration` 一列**——不碰 `object_keys`、不重算历史 `alignment_tasks.mapping`、不动任何 `split_tasks`；重标定只影响此后新发起的对齐/分段。**合并语义**：只更新给出的组，省略的组保持原值，显式 `null` 清除该组。校验：`offset_seconds` 有限且 ≤±3600s（手误护栏）；`roi` 四数有限、`w/h>0`、**且必须落在焊缝照片真实像素范围内**（写入前下载图片读一次宽高，读不到则 400 拒绝）；`excluded` 必须是布尔值。**`excluded=true` 时不需要 ROI，也不下载图片做越界校验**；给 `{roi}` 时服务端显式写 `excluded=false`（撤销"不参与"靠整组替换生效） | body: `{video?: {offset_seconds}, seam_image?: {roi?:{x,y,w,h}, excluded?:bool}} \| null` |
| POST | `/api/v1/welds/{weld_id}/versions/{version_id}/alignment-tasks` | 提交多模态对齐任务（**异步**；成功后自动生成新版本 `action=时间对齐` 并更新 `latest_version_id`）。服务端读取 v1.0 标定参与映射计算，并**按 `t_video = t_signal - offset` 抽关键帧** | body: `modalities[]` |
| GET | `/api/v1/alignment-tasks/{task_id}` | 对齐任务状态/结果（**已真实化**）：`result` 内嵌 `events`（真实信号 `detect_events`，无导入回退生成并以 `event_source=real\|generated` 如实标注）、`tracks[]`（每条 `{channel, modality, availability(available\|generated\|unavailable), source, aligned, asset, object_key, metadata, reason}`，**部分成功语义**——缺失模态不阻塞任务；**2026-09-22 起 `aligned = available && calibrated`**，未标定 offset 的视频轨为 `false` + reason「时间零点未标定」，不再无条件声称已对齐）、`mapping`（**2026-09-22 新增，可执行坐标映射**：`{schema_version, unified_axis{unit,origin,duration}, event_bounds{start,end}, mappings{timeseries(identity) / video(linear: offset_seconds,fps,duration) / seam_image(arc_length: object_key,roi,speed_source,arc_profile) / audio(none)}}`，各模态含 `available`/`calibrated`/`reason`；供分段任务把时间窗换算到视频帧与焊缝图像素，见设计文档 §3.1）、`assets[]`（真实产物：`timeseries.csv`/`timeseries_weld.csv`/`keyframes/{event}.jpg`/`mapping.json`/`tracks.json`，经 `files.getFileUrl` 下载；视频不重编码，前端播放 `track.object_key` 指向的 raw 原始对象） | 轮询（Job 结构） |
