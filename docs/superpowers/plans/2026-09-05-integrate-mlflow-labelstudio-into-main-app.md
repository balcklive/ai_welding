...
> **进度（2026-09-06）**：SDK + 集成骨架已用**真实 LS + 真实 MySQL + 真实 MinIO 走通 e2e**（`backend/scripts/premise_validation/e2e_ls_roundtrip.py`：真样本→推 LS→SDK 标注→**本机模拟 webhook** 回写→幂等→清理，BOX/POLY 两路）。基于 e2e 已闭环下列 TODO：SDK 方法名（`dir(client)` 核对 `tasks.create/get/list/update/delete`、`annotations.create`、`projects.get`）、`convert_region` 真实 region 类型串（**`rectanglelabels`/`polygonlabels`/`timeserieslabels`**，坐标 0-100% 以 `original_width/height` 换算像素）、`refresh_task_media`（保留 data 其它键）、`_media_data_for` 分派（image/csv）、annotator 取 LS 用户名。离线回归 `tests/test_labelstudio_integration.py`（mock SDK，11 项）。迁移 `0013`/`0014` 已落地真实库；`label_categories` 补熔池第 6 类（决策 4）；`load_real_examples` 改缺陷白名单折叠（决策 6）。**剩余（2026-09-06 后端接线已落地）**：后端 `annotation` handler 已接 LS——`mode=on` 时走 `annotation_ls.prepare_ls_task`（归位样本→推流→置 `pending_ls` 等待态，job 保持 running）、`handle_annotation_event` → `_maybe_complete_task`（全部样本回写才任务 synced + job succeeded），`off`/不可达回退模拟（`jobs/annotation.py` + `services/annotation_ls.py`，测试 `tests/test_labelstudio_handler.py`）。**前端嵌入（图像模式）已落地**：`src/features/annotation/LabelStudioEmbed.tsx` + `src/hooks/useLabelStudioTask.ts` + `api/analysis.ts::getLabelStudioTask`（`ls_status∈{pending_ls,annotating}`→iframe 嵌 LS 项目页；synced→只读；legacy/off→Annotorious 画布）。**2026-09-06 服务器已配 LS 并实机验通闭环**：服务器 `.env` 补 `LABEL_STUDIO_MODE=on` + 内网/公网 URL + webhook base + webhook secret + 项目映射 3/4/5，模式=on；LS 侧建 3 个**项目级 webhook**（3/4/5，回调 `http://172.18.0.1:8223/api/v1/labelstudio/webhook`，带 `X-LS-Secret` 头，订阅 `ANNOTATION_CREATED/UPDATED`）；LS PAT 已轮换（`.env.api-key` 换新 token，旧 token 经 `tokens.rotate` 失效）。实机闭环：主应用建任务→推 LS→LS 内 SDK 标→webhook→写回 `annotations`→任务 `synced`→主应用只读（已验证）。**模态现状（2026-09-06）**：仅**图像**完整集成 LS（bbox→项目3，闭环验通）；**视频（polygon→项目4）与时序（segment→项目5）仅为映射/convert/项目就绪，但媒体未导出**——视频锚点/帧样本、信号锚点样本均**无 `object_keys`** → `_media_data_for` 返回 None → 不推 LS，暂走主应用自研画布过渡（决策：signal/video 留旧画布）。

> **下一步（待实施，2026-09-06 已列入计划，不阻塞已完成项）**：把视频/时序标注也集成进 LS——仅缺**媒体导出桥**，复用现有抽帧/信号能力，不自研标注内核。
> - [ ] **视频 → 项目4（polygon·熔池分割）**：从视频按需抽帧为图片（复用 `media_probe.analyze_video`），作为视频帧/锚点样本的 `object_keys` 推 LS（`meta.mode='frame'` 已映射 kind=polygon → `project_for→4`）；验收：视频任务 → LS 项目4 出现 task、标 polygon → webhook 写回 → 只读、`kind=polygon`/`points` 无掩膜转换。
> - [ ] **时序 → 项目5（segment·区间分段）**：把信号 Parquet 抽核心通道（`t,cur,vol,gas,wir`，可降采样、显式 `time` 秒列）导成 LS TimeSeries `$csv` 媒体（`media_field_for('segment')='csv'` 已就绪；**注意** `schema_version=2` 动态 16 列，整 CSV 直推会让 LS 面板塞满近常值六轴/熔池列，须抽核心 4 或 +weld_speed）；验收：时序任务 → LS 项目5 task、CSV 可标区间、回写 → `segment`(start_time/end_time)。
> - [ ] 后台对账心跳定时调度（当前 `POST /labelstudio/sync` 手动触发；webhook 已是主回写路径，对账作兜底）。
> - [ ]（同步契约）媒体导出新增的对象键/CSV 前缀回写 `docs/OSS存储设计.md`，与 `docs/API接口清单.md`、`docs/数据库设计.md` 保持一致。
