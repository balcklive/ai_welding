...
- `deploy-docker.yml`：`main` 分支通过前端/后端校验后，SSH 到服务器拉取代码、本地构建 Docker 镜像并替换 FastAPI 容器；服务器使用 Secrets `SSH_HOST`/`SSH_PORT`/`SSH_USER`/`SSH_PRIVATE_KEY`，以及 Variables `DEPLOY_PATH`/`APP_CONTAINER_NAME`/`HOST_PORT`；默认部署目录为 `/home/wwwroot/code/ai_welding`，默认宿主机端口为 `8223`。
