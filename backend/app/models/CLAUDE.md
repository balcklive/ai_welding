...
SQLModel 表类（全部 23 张，`table=True`）。当前进度：Task 2（全部模型 + `__init__.py` 导出）已完成。
...
- `analysis.py`：`AlignmentTask`(§3.7)、`SplitTask`(§3.8)、`Sample`(§3.9)、`AnnotationTask`(§3.10)、`Annotation`(§3.11)、`LabelCategory`(§3.12)、`FeatureExtraction`(§3.13)。
