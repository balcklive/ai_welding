...
- `__init__.py`：空包。
- `config.py`：pydantic-settings `Settings` + 模块级单例 `settings`。字段覆盖 MinIO/MySQL/Auth/API 日志；`mysql_url` property 拼 `mysql+pymysql://...`；`job_executor_enabled` 允许候选容器禁用任务消费。**2026-09 新增 `minio_server_endpoint`**（`MINIO_SERVER_ENDPOINT`，服务端内网端点，空则回退公网——配合 storage 双端点内网化，见 `storage/CLAUDE.md`）。**2026-09 另新增 Label Studio 配置**（`label_studio_mode`/`internal_url`/`public_url`/`webhook_base`/`api_key`/`webhook_secret`/`presign_expires` + `ls_project_detection/segmentation/timeseries`，见 `integrations/labelstudio.py`；敏感值只进服务器 `.env`）。
...
- **2026-09 系统设置默认字典**：`seed_reference_data(session)` 除 6 个标签类别外（补齐 `sort_order`/`active`），
  ...
  **2026-09-22**：追加第 7 组 `defect_category` 的 7 项默认缺陷词表（气孔/未焊透/焊穿/咬边/裂纹/
  成形不良/其他，与迁移 `0020` 同一份默认值）——段级标注工作台的候选就来自它。
  同组同名项已存在则**不覆盖**，管理员改名/停用不会被重启回灌；同样的默认值也写在迁移 `0015`/`0020`（双写入，均幂等）。
  **2026-09-24**：新增 `material`（板材材质 6 项）与 `thickness`（板材厚度 9 项：3/4/5/6/8/10/12/16/20）
  ——登记页这两个字段的候选下拉。**只写在本文件，不配迁移**：`seed_reference_data` 每次启动幂等跑，
  已部署库重启即生效，而迁移那一份是为建表/建新组配套的（这里无表结构变更）。
  **`thickness` 候选必须是纯数字**——加 `mm` 会与提交时的剥单位 + `0.1–200` 校验打架。
