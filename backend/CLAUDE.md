...
- `tests/`：pytest 测试（`uv run pytest`，SQLite 内存 + TestClient，不连远程库）。`test_welds.py` 为 Task 10（Welds CRUD），覆盖登记事务/编号递增/列表筛选分页/版本链/核验级联/401；`test_dsp.py` + `test_analysis.py` 为 Task 11（真实 DSP 纯函数 + analysis 端点：candidates/signals/六 mode/result/401）、`test_alignment.py` 为 Task 13（对齐任务：创建→run_job→succeeded + 自动时间对齐版本 / 失败→failed / 404 / 401）、`test_datasets.py` 为 Task 15（数据集 CRUD / dimensions/readiness / 版本 / 构建任务→按焊缝分组防泄漏 / lineage / 404 / 401）、`test_models.py` 为 Task 2 模型建表/索引一致性 + **Task 16 模型中心 API**（列表汇总/详情/新建 409/状态流转/训练→自动生成实验版本 + weights.pt / 测试→混淆矩阵 / 推理→boxes / 日志 / 404 / 401）。
