...
- `tests/`：pytest 测试（`uv run pytest`，SQLite 内存 + TestClient，不连远程库）。
  **⚠️ 现状（2026-09-14 核对）：下面这段列出的多个文件已不在仓库里。** `9c68a83 "fix dataset browsing and remove obsolete demo tests"` 删除了 **15 个测试文件共 8023 行**（`test_welds`/`test_models`/`test_split_annotation`/`test_datasets`/`test_alignment`/`test_reports`/`test_dashboard`/`test_features`/`test_analysis`/`test_authz`/`test_audit_routes`/`test_media_prep`/`test_signal_ingest`/`test_signals_downsample`），磁盘上现为 **23 个文件 / 224 用例**（`ls tests/test_*.py` 为准；2026-09-14 数据管理改造第二轮补齐后由 160 涨到 224，新增 `test_registration_t4b.py` 并把 `test_dataset_members.py` / `test_dataset_build_e2e.py` 扩写）。也就是说"Welds CRUD / 数据集构建 / 训练 / 切分标注 / 权鉴"这些核心链路的**离线回归目前是缺的**（T11 之前 `run_build` 完全没有测试）。要恢复可从 `git checkout 9c68a83^ -- backend/tests/<file>` 取回，但那些用例断言了不少已被删除的演示/兜底行为，取回后需逐条核对。**新增/改动核心链路时请补测试**（如 `tests/test_dataset_members.py` 就是为 T11 补的）。以下描述保留为**历史记录**，勿当现状。`test_welds.py` 为 Task 10（Welds CRUD），覆盖登记事务/编号递增/列表筛选分页/版本链/核验级联/401；`test_dsp.py` + `test_analysis.py` 为 Task 11（真实 DSP 纯函数 + analysis 端点：candidates/signals/六 mode/result/401）、`test_alignment.py` 为 Task 13（对齐真实化内核：创建→run_job→succeeded + 自动时间对齐版本 / **generated 回退 + 真实信号 ingest + 真实视频 ffmpeg 探测/关键帧** / 失败→failed / 404 / 401）、`test_media_probe.py` 为 **ffmpeg 媒体探测/关键帧单测**（不连 DB）、`test_datasets.py` 为 Task 15（数据集 CRUD / dimensions/readiness / 版本 / **版本成员分页接口 items（total/页切片/真实跨数据集版本→40402）** / 构建任务→按焊缝分组防泄漏 / lineage / 404 / 401）、`test_models.py` 为 Task 2 模型建表/索引一致性 + **Task 16 模型中心 API**（列表汇总/详情/新建 409/状态流转/训练→自动生成实验版本 + weights.pt / 测试→混淆矩阵 / 推理→boxes / 日志 / 404 / 401）、`test_reports.py` 为 **Task 17 通用导出**（validation json→score 93.3 + 15 条规则 / validation pdf→`%PDF` 头 / data-list 全量+按标识过滤 / analysis 通用模板 / 未知 type·format 400 / 实体缺失 404 / 401，monkeypatch `app.storage.get_storage` → FakeStorage）、`test_weld_delete.py` + `test_dataset_delete.py` 为 **2026-08-29 删除安全**（空数据集/无版本焊缝可删、有焊缝或训练/测试引用时拒绝；含版本级联清理）、`test_torch_training.py` + `test_mlflow_integration.py` 为 **2026-08-29 真实训练**（CPU 训练特征/指标/权重序列化、MLflow off 模式不炸且记录 helper 可空 run）。
