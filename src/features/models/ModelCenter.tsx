...
import { READY_TEXT } from '../../shared/lib/terms';
...
/** 模型类型 → 图标（模型卡片按 type 区分；未识别类型回退 Cpu）。 */
const MODEL_TYPE_ICONS: Record<string, typeof Cpu> = {
  时序分类: Activity,
  目标检测: Target,
  语义分割: ScanLine,
  多模态回归: BarChart3,
};
function ModelTypeLogo({ type }: { type: string | null | undefined }) {
  const Icon = (type && MODEL_TYPE_ICONS[type]) || Cpu;
  return <div className="model-logo"><Icon size={17} /></div>;
}
...
export function DatasetBuild() {
  ...
  const sourceMeta: Record<string, { label: string; desc: string }> = {
    ...
    annotation_task: { label: '已标注切片', desc: '仅纳入已完成标注的切片' },
  };
  return <div className="page-wrap"><PageIntro eyebrow="模型研发中心" title="训练数据准备" description="基于数据管理的数据集，筛选可用于模型训练的样本并生成固定版本。" action={<Toolbar secondary="导出报告" />} /><div className="build-steps" aria-label="训练数据准备流程"><span className="current"><b>1</b>选择数据集</span><i /> <span><b>2</b>设定切片范围</span><i /> <span><b>3</b>生成训练数据集</span></div><div className="build-layout"><section className="panel build-config"><div className="panel-heading"><div><h2>准备条件</h2><p>配置完成后生成一份新的训练数据版本</p></div><Box size={17} /></div>{loading ? <p className="dataset-empty-state" role="status">正在读取数据管理的数据集…</p> : <><div className="form-block"><label className="form-label">1. 选择数据集</label><p className="form-help build-form-intro">选择数据管理中已有的数据集，准备动作不会修改原始数据。</p><select className="native-select" value={datasetId ?? ''} onChange={(e) => setDatasetId(e.target.value ? Number(e.target.value) : null)}><option value="">请选择数据集</option>{datasets.map((d) => <option key={d.id} value={d.id}>{d.name} · {d.version ?? '未建版本'}</option>)}</select></div><div className="form-block"><label className="form-label">2. 选择切片范围</label><div className="build-source">{(['manual', 'split_task', 'annotation_task'] as const).map((key) => <label className={source === key ? 'chosen' : ''} key={key}><input type="radio" name="build-source" checked={source === key} onChange={() => setSource(key)} /><span><strong>{sourceMeta[key].label}</strong><small>{sourceMeta[key].desc}</small></span>{source === key && <Check size={15} />}</label>)}</div></div></>}<div className="split-ratio-note"><div className="rule-title"><GitBranch size={15} />自动划分规则</div><b>训练 80% <em>/</em> 验证 10% <em>/</em> 测试 10%</b><small>按样本分组，同一个样本的切片不会同时进入训练集和测试集，避免数据泄漏。</small></div><button className="full-button" onClick={handleBuild} disabled={datasetId == null || buildJobId != null}>{buildJobId != null ? <><Activity size={16} />正在生成 · {progress}%</> : <><GitBranch size={16} />生成训练数据集</>}</button>{buildError && <p className="dataset-empty-state" role="alert">生成失败：{buildError} 请检查样本范围后重试。</p>}</section><section className="panel build-result"><div className="panel-heading"><div><h2>生成结果</h2><p>{buildJobId != null ? '正在生成训练数据版本…' : buildResult ? '最近一次生成结果' : '完成左侧配置后，这里会显示数据划分结果'}</p></div><StatusPill tone={buildStatus === 'failed' ? 'red' : buildStatus === 'running' ? 'orange' : buildResult ? 'green' : 'muted'}>{buildStatus === 'succeeded' ? '已生成' : buildStatus === 'failed' ? '生成失败' : buildStatus === 'running' ? '生成中' : '未开始'}</StatusPill></div>{buildResult && split ? <><div className="result-context"><CheckCircle2 size={16} /><span>训练数据集已生成，可在“新建训练”中使用</span></div><div className="build-splitbar"><i className="train" style={{ width: slicePct(split.train) }} /><i className="val" style={{ width: slicePct(split.val) }} /><i className="test" style={{ width: slicePct(split.test) }} /></div><div className="build-split-legend"><span><i className="train" />训练集 <b>{split.train ?? 0}</b></span><span><i className="val" />验证集 <b>{split.val ?? 0}</b></span><span><i className="test" />测试集 <b>{split.test ?? 0}</b></span></div><div className="build-stat-row"><div><span>切片总数</span><strong>{buildResult.item_count.toLocaleString()}</strong></div><div><span>有效切片占比</span><strong>{qualityPct ?? '—'}</strong></div><div><span>快照文件</span><strong>{buildResult.snapshot_id ? buildResult.snapshot_id.slice(0, 8) : '—'}</strong></div></div></> : <div className="build-state"><div className="empty-steps"><span><Database size={17} />选择数据集</span><span><SlidersHorizontal size={17} />选择切片范围</span><span><GitBranch size={17} />生成版本</span></div><strong>{buildJobId != null ? '正在生成训练数据版本' : '还没有训练数据版本'}</strong><p>{buildJobId != null ? `系统正在按样本分组生成，当前进度 ${progress}%` : '配置左侧两项条件后，点击“生成训练数据集”即可开始。'}</p></div>}</section></div></div>;
}
...
export function TrainingDataPreparation() {
  ...
  useEffect(() => {
    ...
    if (buildStatus === 'failed') setBuildError('请检查切片范围与核验状态后重试');
  ...
  return <div className="page-wrap">
    <PageIntro eyebrow="模型研发中心" title="生成训练数据版本" description="从数据管理选择一个已有数据集，筛选切片并生成供模型训练使用的数据集版本。" action={<Toolbar secondary="导出报告" />} />
    <div className="build-steps" aria-label="训练数据准备流程"><span className="current"><b>1</b>选择输入数据集</span><i /><span><b>2</b>筛选切片范围</span><i /><span><b>3</b>生成训练版本</span></div>
    <div className="build-layout">
      <section className="panel build-config"><div className="panel-heading"><div><h2>生成条件</h2><p>创建一份新的固定训练数据版本</p></div><Box size={17} /></div>
        {loading ? <p className="dataset-empty-state" role="status">正在读取数据管理的数据集…</p> : datasets.length === 0 ? <div className="build-no-dataset"><Database size={22} /><strong>暂无可用输入数据集</strong><p>请先在数据管理创建数据集并生成版本。</p></div> : <>
          ...
          {isTrainable ? <div className="form-block"><label className="form-label">2. 切片纳入范围</label><div className="build-source">{(['manual', 'split_task', 'annotation_task'] as const).map((key) => <label className={source === key ? 'chosen' : ''} key={key}><input type="radio" name="training-source" checked={source === key} onChange={() => setSource(key)} /><span><strong>{sourceMeta[key].label}</strong><small>{sourceMeta[key].desc}</small></span>{source === key && <Check size={15} />}</label>)}</div></div> : dataset && <div className="build-blocked" role="alert"><AlertTriangle size={18} /><div><strong>当前数据集还不能生成训练数据版本</strong><p>数据集仍在标注中或尚未通过适配检查，请完成标注并通过检查后再生成训练数据版本。</p></div></div>}
        </>}
        {isTrainable && <div className="split-ratio-note"><div className="rule-title"><GitBranch size={15} />3. 训练数据划分</div><b>训练 80% <em>/</em> 验证 10% <em>/</em> 测试 10%</b><small>按样本分组，同一个样本的切片不会同时进入训练集和测试集，避免数据泄漏。</small></div>}
...
export function Training() {
  ...
  const handleStart = () => {
    ...
    createTrainingTask({
      ...
      .catch((err) => { setTrainingError(err instanceof Error ? err.message : '训练任务创建失败，请检查数据集版本后重试'); console.warn('[training] createTrainingTask failed', err); });
  };
  const handleLogs = () => {
    if (!jobId) return;
