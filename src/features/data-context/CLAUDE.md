...
- 调用谁：`src/hooks/usePagedWelds`（样本候选分页搜索）、`src/api/welds`（getWeld/listVersions/createVersion/runValidation）、`src/api/datasets`（listDatasetOptions）、`src/api/files`（presignUpload/putFileDirect）、`src/features/datasets/weldRows`（toWeldRow）、`src/features/versions/VersionDetailDrawer`、`src/shared/components`（StatusPill/ErrorState）、`src/shared/lib/errors`（toUserMessage）。
...
- **跨域上下文（2026-09-14 决定，R8）**：点侧边栏「数据管理」会先落到数据集列表，而列表页挂载时的 `datasetHomeKey` effect 会清空已选数据上下文 → "在「选择数据」选好样本 → 切到核验/版本"这条路径要重选一次。**决定维持现状**：保留上下文需要同时改三个互相耦合的 effect（列表页 view 重置、`selectedDatasetId` 驱动的自动进概览、全局上下文清空），收益只是省一次重选；T5 的守卫已给出一键恢复（「在本页选择数据」聚焦并高亮选择器）。要改的话按方案 §12 的方向①另立任务。
