...
- `datasets/`：数据中心·数据集（列表→概览→快照成员→成员详情 + 原始波形/媒体预览 + 数据集/焊缝删除）。
- `registration/`：数据中心·数据上传（新建操作，不要求先选数据）。
- `validation/`：数据中心·数据核验（15 项规则）。
- `versions/`：`VersionDetailDrawer` 版本详情抽屉（焊缝版本 / 数据集快照两种 mode）。
- `analysis/`：分析与标注·信号分析（`AdvancedWeldAnalysis` + 六种图表 + `signals/chartData` 演示坐标系工具）。
