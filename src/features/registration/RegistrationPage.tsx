...
import { CSV_INGEST_CONFLICT_CODE, attachRawFiles, createRegistration, listWelds, reimportSignals } from '../../api/welds';
...
/** T4.4：识别「CSV 已存在导入任务」这个**其实是成功**的 409（后端独立错误码，见 `api/welds.ts`）。 */
const isCsvIngestConflict = (err: unknown): boolean =>
  typeof err === 'object' && err !== null && (err as { code?: number }).code === CSV_INGEST_CONFLICT_CODE;

/** 表单字段 → 中文名（弹窗与提示共用）。 */
const FIELD_LABELS: Record<string, string> = {
  source: '数据来源', collected_at: '采集时间', weld_name: '样本名称（焊缝 / 批次）',
  product: '关联产品信息', machine: '焊机型号', weld_method: '焊接方法', material: '板材材质',
  thickness: '板材厚度', current_a: '电流', voltage_v: '电压', sample_rate: '采样频率',
...
const DEFAULTABLE_FIELDS = ['source', 'machine', 'weld_method', 'material', 'thickness', 'current_a', 'voltage_v', 'sample_rate', 'product'] as const;

/** 可以走默认值的字段（`dataset_id` 由数据集锁定、`collected_at` 用当前时间，都不在其中）。 */
type DefaultableField = (typeof DEFAULTABLE_FIELDS)[number];

/** 上次登记的值：localStorage 里一律按字符串存（表单里也都是字符串，数字字段提交时才转）。 */
type LastValues = Partial<Record<DefaultableField, string>>;
...
function loadLastValues(): LastValues {
  try {
    const raw = JSON.parse(localStorage.getItem(LAST_VALUES_PREFIX + currentUserKey()) ?? '{}') as Record<string, unknown>;
    const picked: LastValues = {};
    // 白名单过滤：老版本可能存过 `current_voltage`（D7 前的字段）或别的键，不带进表单
    for (const field of DEFAULTABLE_FIELDS) {
      const value = raw[field];
      if (typeof value === 'string' || typeof value === 'number') picked[field] = String(value);
    }
    return picked;
  } catch {
    return {};
...
function saveLastValues(form: RegistrationForm): void {
  ...
  for (const field of DEFAULTABLE_FIELDS) {
    ...
    if (value !== null && value !== undefined && String(value).trim()) picked[field] = String(value);
  }
  try {
...
export function RegistrationPage({ navigate, lockedDatasetId: inheritedDatasetId, setSelectedDatasetId, setSelectedDataId }: {
  ...
  // T4.4/R2：**已直传成功**的对象键（按上传区）。部分失败重试时只重传失败的那个区，
  // 不把已经传完的文件再传一遍（几十 MB 的视频重传一次是实打实的带宽浪费）。
  const [uploadedKeys, setUploadedKeys] = useState<Partial<Record<UploadZoneKey, string>>>({});
  ...
  // T4.4/R2：登记链路状态（待上传 / 导入中 / 导入失败 / 可分析）——由后端推导，不靠前端记忆。
  const ingest = useIngestStatus(result ? String(result.id) : null);
   ...
   */
  const applyDefaults = (patch: LastValues) => {
    const next = { ...formRef.current };
    const applied: [DefaultableField, string][] = [];
    for (const [key, value] of Object.entries(patch) as [DefaultableField, string | undefined][]) {
    ...
    setForm(next);
    setDefaultValues((prev) => ({ ...prev, ...Object.fromEntries(applied) }));
  };
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      // T4.2/D7：电流/电压按数字提交（后端量程校验是 float），厚度剥单位后提交
      const payload: RegistrationForm = {
        ...form,
        current_a: Number(form.current_a),
        voltage_v: Number(form.voltage_v),
        thickness: String(form.thickness ?? '').replace(/mm$/i, '').trim(),
      };
      const reg = regRef.current ?? await createRegistration(payload);
      regRef.current = reg;
      const keys: string[] = [];
      const freshKeys: Partial<Record<UploadZoneKey, string>> = {};
      for (const zone of UPLOAD_ZONES) {
        const file = files[zone.key];
        if (!file) continue;
        // T4.4/R2：上一轮已经传成功的文件直接复用对象键，不再重传。
        const done = uploadedKeys[zone.key];
        if (done) {
          keys.push(done);
        ...
        try {
          ...
          freshKeys[zone.key] = object_key;
          setUploads((prev) => ({ ...prev, [zone.key]: { status: 'done', fileName: file.name } }));
        } catch (err) {
          console.warn('[registration] file upload failed', err);
          // 已传成功的区记下来：下一次重试只补失败的那个
          setUploadedKeys((prev) => ({ ...prev, ...freshKeys }));
          setUploads((prev) => ({ ...prev, [zone.key]: { status: 'error', fileName: file.name } }));
          throw new Error(`文件 ${file.name} 上传失败，请重试（已上传的文件不会重复上传）`);
        }
        setUploadedKeys((prev) => ({ ...prev, ...freshKeys }));
      }
      try {
        ...
        // T4.4/R2：CSV 重复挂载（40901）说明**上一次挂载已经成功、只是响应丢了**——
        // 按"已挂载成功"继续进导入态，不要当失败卡住（这是线上最常见的断链点）。
        if (!isCsvIngestConflict(err)) {
          console.warn('[registration] attachRawFiles failed', err);
          throw new Error('文件已上传但关联失败，请重试（不会重复登记）');
        }
        console.warn('[registration] 挂载已存在（CSV 导入任务重复），按已挂载成功继续');
      }
      ...
      // T3.1：按场景翻译成人话（字段级 422 / 网络 / 409 业务冲突各有分支）。
      setRegError(toUserMessage(err, '登记数据').reason ?? toUserMessage(err, '登记数据').message);
    } finally {
      setSubmitting(false);
    }
  };

  /** 导入失败后的「重新导入」（T4.4）：后端清掉 failed 行重新入队，用户不必删库。 */
  const handleReimport = async () => {
    if (!result || reimporting) return;
    setReimporting(true);
    try {
      await reimportSignals(String(result.id));
      ingest.refresh();
    } catch (err) {
      console.warn('[registration] reimport failed', err);
      setRegError(toUserMessage(err, '重新导入').reason ?? toUserMessage(err, '重新导入').message);
    } finally {
  ...
  /**
   * T4.4：提交失败后若登记已经建好（`regRef`），用户改了内容再提交会与已建登记不一致。
   * 一期不做 PATCH 回填（属 T13 编辑锁范畴），只给一个显式出口：放弃这条登记重新建。
   * 已直传的文件仍是 MinIO 里的有效对象，重新登记时直接复用，不重传。
   */
  ...
  const startNext = () => {
    const fresh: RegistrationForm = { dataset_id: lockedDatasetId ?? 0, source: '', collected_at: toLocalInputValue(new Date()), weld_name: '', product: '', machine: '', weld_method: '', material: '', thickness: '', current_a: '', voltage_v: '', sample_rate: '', wire_feed_speed: '', welding_speed: '' };
  ...
  // T4.2 量程（与后端 Field 约束同一套）：前端即时挡住，不必等 422
  const inRange = (value: unknown, min: number, max: number) => {
    const num = Number(value);
    return String(value ?? '').trim() !== '' && Number.isFinite(num) && num >= min && num <= max;
  };
  const missingFields = [
    ...
    { key: 'machine', label: '焊机型号', ok: !!form.machine?.trim() },
    { key: 'weld_method', label: '焊接方法', ok: !!form.weld_method?.trim() },
    { key: 'material', label: '板材材质', ok: !!form.material?.trim() },
    { key: 'thickness', label: '板材厚度（0.1–200 mm）', ok: inRange(String(form.thickness ?? '').replace(/mm$/i, ''), 0.1, 200) },
    { key: 'current_a', label: '电流（1–2000 A）', ok: inRange(form.current_a, 1, 2000) },
    { key: 'voltage_v', label: '电压（1–200 V）', ok: inRange(form.voltage_v, 1, 200) },
    { key: 'sample_rate', label: '采样频率', ok: !!form.sample_rate?.trim() },
  ...
  const handleFile = (key: UploadZoneKey, event: ChangeEvent<HTMLInputElement>) => {
    ...
    // 换了文件就不能再复用旧的对象键——否则会把**上一个**文件挂到这次登记上
  ...
  return <div className="page-wrap">
    ...
    <div className="registration-layout">
      {result && <section className="panel registration-result">
        ...
        <p>{form.weld_name || '样本'} 已归档到「{lockedDataset?.name ?? TERMS.dataset}」。登记成功只代表台账建好了，核验与分析要等下面的信号导入完成。</p>
        <div className="registration-states">
          <p className="registration-state" role="status"><span>信号导入</span><strong>{ingestStatusText(ingest.status)}</strong></p>
          {ingest.status?.status === 'failed' && <div className="registration-state-actions">
            <span className="toolbar-error" role="alert">{ingest.status.csv_failed.length} 个文件导入失败：{ingest.status.csv_failed[0]?.source_object_key}{ingest.status.csv_failed[0]?.message ? `（${ingest.status.csv_failed[0].message}）` : ''}</span>
            <button className="outline-button" onClick={() => void handleReimport()} disabled={reimporting}><RefreshCw size={14} />{reimporting ? '重新导入中…' : '重新导入'}</button>
          </div>}
          {buildJobId && <p className="registration-state" role="status"><span>数据集版本构建</span><strong>{buildStatus === 'succeeded' ? '已完成' : buildStatus === 'failed' ? '构建失败，可在数据集页面点「重新构建」' : '构建中…'}</strong></p>}
        </div>
      ...
      {!result && <section className="panel form-panel">
        ...
        </div> : <fieldset className="form-fieldset" disabled={submitting}>
        ...
        {missingHint && <span className="toolbar-error" role="alert">{missingHint}</span>}
        </fieldset>}
        {/* T4.4/R2：失败态在 fieldset 之外——登记已建好时提交按钮是禁用的，提示与出口仍要可点 */}
        {lockedDatasetId != null && regError && <div className="registration-error" role="alert">
          <AlertTriangle size={15} />
          <span>{regError}</span>
          {regRef.current && <span className="registration-error-hint">当前登记 <strong>{regRef.current.registration_no}</strong> 已创建，已上传的文件不会重复上传；修改登记内容需重新登记。</span>}
          <div className="registration-error-actions">
            <button className="primary-button" onClick={handleSubmitClick} disabled={submitting}>重试提交</button>
            {regRef.current && <button className="ghost-button" onClick={abandonRegistration}>放弃本次登记，重新登记</button>}
          </div>
        </div>}
      </section>}
