...
- `SettingsPage.tsx`：`SettingsPage()` —— 可选项字典管理页。**面板按后端 `OPTION_GROUPS` 动态渲染**（组名/label/说明/是否支持颜色全部来自后端，加一组只改后端；2026-09 至今 6 组 → 9 组两次扩容本页均零改动。唯一的 key 特判是新增框 placeholder 对 `product` 的示例文案）。每组支持：
