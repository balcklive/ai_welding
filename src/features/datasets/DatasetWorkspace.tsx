...
function DatasetRecords({ dataset, versionId, onBack, onVersionChange, onSelectRecord }: { dataset: DatasetRow; versionId: number; onBack: () => void; onVersionChange: (versionId: number) => void; onSelectRecord: (row: DatasetItemRow) => void }) {
  ...
  useEffect(() => { setPage(1); }, [query, split, versionId]);
  useEffect(() => {
    let cancelled = false;
    setLoading(true);
    setItemsError(null);
    listDatasetVersionItems(dataset.id, versionId, { q: query.trim() || undefined, split: split || undefined, page, page_size: PAGE_SIZE })
    ...
    return () => { cancelled = true; };
  }, [dataset.id, versionId, query, split, page, reloadKey]);
