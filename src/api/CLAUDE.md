...
- `types.ts`：全部实体/请求体类型，字段名/形状**逐字对齐后端 `*_payload` 序列化**与 `docs/API接口清单.md` §2、§1.4、§1.5。后端为契约：`Page<T>={items,total,page,page_size}`、`Job<T>={id,type,status,progress,result,error,created_at,finished_at}`、`Registration` = `DataRecord` 别名（`/registrations` 返回同构 record_payload）。Task 19 未新增类型（全部签名所需类型已在 `types.ts` 就绪）。`SignalQuery` 新增 `max_points/start/end`（波形预览服务端 min-max 抽稀 + 时间窗）；`SignalChannel` 新增可选 `times[]`（服务端抽稀时的采样秒时刻，min-max 选点非均匀，前端须按 [t,v] 画点）。
