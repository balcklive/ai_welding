...
type Workspace = 'overview' | 'management' | 'registration' | 'analysis' | 'validation' | 'alignment' | 'annotation' | 'training' | 'model-test';
const navItems: { id: Workspace; label: string; icon: typeof BarChart3 }[] = [
  { id: 'overview', label: '数据总览', icon: BarChart3 },
  { id: 'management', label: '数据管理', icon: Database },
  { id: 'registration', label: '数据登记', icon: FileCheck2 },
  { id: 'analysis', label: '焊缝分析', icon: Waves },
  { id: 'validation', label: '数据核验', icon: ClipboardCheck },
  { id: 'alignment', label: '对齐与切分', icon: GitBranch },
  { id: 'annotation', label: '数据标注', icon: Tag },
  { id: 'training', label: '模型训练', icon: TrainFront },
  { id: 'model-test', label: '模型测试', icon: ScanLine },
];
const projects = [
...
function App() {
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <div className="sidebar-divider" /><div className="workspace-label">项目</div><div className="project-list">{projects.map((project, index) => <button key={project.name} className={`project-item ${activeProject === index ? 'selected' : ''}`} onClick={() => setActiveProject(index)}><span className={`project-dot ${project.tone}`} /><span>{project.name.split(' · ')[0]}</span><span className="project-count">{project.count}</span></button>)}</div>
      <button className="new-project"><Plus size={16} /> 新建数据项目</button><div className="sidebar-bottom"><button className="nav-item"><Settings2 size={18} /><span>系统设置</span></button><div className="user-card"><div className="avatar">林</div><div><strong>林工</strong><span>管理员</span></div><MoreHorizontal size={16} /></div></div>
    </aside>
    <main className="main-content"><header className="topbar"><div className="breadcrumbs"><span>工作空间</span><ChevronDown size={14} /><strong>{navItems.find((item) => item.id === workspace)?.label}</strong></div><div className="top-actions"><div className="search-box"><Search size={16} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索数据集、模型..." /></div><button className="icon-button"><Bell size={18} /><i /></button><div className="top-avatar">L</div></div></header>
      ...
      {workspace === 'management' && <Management setWorkspace={setWorkspace} />}
      {workspace === 'registration' && <Registration />}
      {workspace === 'analysis' && <WeldAnalysis />}
      {workspace === 'validation' && <Validation />}
      {workspace === 'alignment' && <Alignment />}
      {workspace === 'annotation' && <Annotation saved={saved} setSaved={setSaved} selectedLabels={selectedLabels} toggleLabel={toggleLabel} />}
      {workspace === 'training' && <Training isTraining={isTraining} setIsTraining={setIsTraining} />}
      {workspace === 'model-test' && <ModelTest />}
    </main>
  </div>;
}
...
function Overview({ activeProject, filteredProjects, setWorkspace }: { activeProject: number; filteredProjects: typeof projects; setWorkspace: (workspace: Workspace) => void }) {
  return <div className="page-wrap"><PageIntro eyebrow="数据资产中心" title="数据总览" description="全面掌握焊接数据资产的规模、来源与质量分布。" action={<button className="primary-button" onClick={() => setWorkspace('annotation')}><Tag size={16} />开始标注</button>} />
...
function Management({ setWorkspace }: { setWorkspace: (workspace: Workspace) => void }) {
  const [selected, setSelected] = useState<string[]>([]);
  const toggle = (id: string) => setSelected((items) => items.includes(id) ? items.filter((item) => item !== id) : [...items, id]);
  return <div className="page-wrap"><PageIntro eyebrow="数据资产中心" title="数据管理" description="统一管理焊缝原始数据、处理版本与标注资产，保持每一条数据可追溯。" action={<Toolbar action="上传数据" />} />
...
function Training({ isTraining, setIsTraining }: { isTraining: boolean; setIsTraining: (isTraining: boolean) => void }) {
