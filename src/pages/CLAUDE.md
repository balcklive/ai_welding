...
前端页面层（契约：`docs/API接口清单.md` §4.3）。当前进度：**Task 20**（最小登录页 `Login.tsx`）。后续接入页面（总览/数据中心/分析/模型中心）逐步落位，UI 仍由 `src/App.tsx` 的单文件大组件承载（勿重构）。
...
- `Login.tsx`：最小登录页（默认导出 `Login`，无 UI 库）。
  ...
  - 视觉：深底 `#102d35` 居中卡片，品牌 `Sparkles` + ForgeLab，与 App 侧边栏一致。
